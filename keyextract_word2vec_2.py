...
def getkeywords_kmeans(data, topK):
    ...
    for index in range(len(vec_words)):  # 候选关键词个数
        ...
        dis = np.linalg.norm(vec_center - cur_wordvec)
    ...
    word_split = [word.decode('utf-8') if isinstance(word, bytes) else word for word in wordlist[:topK]]  # 抽取前topK个词汇并确保是字符串
...
def main():
    ...
    for i in range(len(fileList)):
        ...
        if os.path.isfile(path):
            ...
            artile_keys = getkeywords_kmeans(data,8) # 聚类算法得到当前文件的关键词
            # 根据文件名获得文章id以及标题
    ...
    result.to_csv("result/keys_word2vec.csv", index=False, encoding='utf_8_sig')
